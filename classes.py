...
# TODO: Class still underutilized.
class MCIAnalysis:
    def __init__(
        self,
        dataset,
    ...
    def fit(self):
        ...
        if self.multiple_co_inertia_result is not None:
            for res_dict in self.multiple_co_inertia_result:
                ...
                self.principal_coordinates.append(
                    res_dict.get("column_principal_coordinates")
                )
    ...
    # organises the data into a list with additional information and the data.
    def transform(self):
        if self.multiple_co_inertia_result is not None:
    ...
    # runs mcoia and outputs the results. End of default pipeline.
