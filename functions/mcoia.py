...
from mcioa.functions.mcoa_processing import (
    ktab_util_names,
    normalize_matrix_by_block,
    recalculate,
)


def row_normed_column_projection(
    number_rows,
    number_Datasets,
    n_dim,
    row_weight,
    ...
    for k in range(number_Datasets):
        ...
        squared_values = (
            row_projection.values * row_weight_squared.reshape(-1, 1)
        ) ** 2
...
def calculate_row_projection(
    ...
    for k in range(number_Datasets):
        ...
        scaled_data = (
            projection * analysis_result["SynVar"].values
        ) * row_weight.reshape(-1, 1)
    ...
    covar_df = pd.DataFrame(
        covar**2,
        index=datasets["tab.names"],
        columns=[f"cov2{i + 1}" for i in range(n_dim)],
    )
...
def create_analysis_dataframes(
    ...
    average_view_u,
    v_k,
    multi_block_eigen_data,
    X,
    auxiliary_names,
    n_dim,
):
    analysis_result["lambda"] = pd.DataFrame(
        lambda_matrix,
        index=multi_block_eigen_data["tab_names"],
        columns=[f"lam{i}" for i in range(1, n_dim + 1)],
    )

    analysis_result["SynVar"] = pd.DataFrame(
        np.array(average_view_u).T[:, :n_dim],
        index=X["row.names"],
        columns=[f"SynVar{i}" for i in range(1, n_dim + 1)],
    )

    analysis_result["axis"] = pd.DataFrame(
        np.array(v_k).T[:, :n_dim],
        index=auxiliary_names["col"],
        columns=[f"Axis{i}" for i in range(1, n_dim + 1)],
    )


def multiple_coinertia_analysis(
    datasets, weight_option=None, n_dim=3, data_projected=False
):
    ...
    rank_per_block = list(
        np.repeat(range(1, number_Datasets + 1), multi_block_eigen_data["rank"])
    )
    ...
    if weight_option == "lambda1":
        weight_table = [
            1 / multi_block_eigen_data["eigenvalues"][rank_per_block[i - 1]][0]
            for i in range(1, number_Datasets + 1)
        ]
    ...
    for i in range(min_dimensions):
        ...
        normalized_v = normalize_matrix_by_block(
            vt[0, :], number_Datasets, block_Indicator, dataset_indix
        )

        x_Tilde = recalculate(
            x_Tilde, normalized_v, number_Datasets, block_Indicator, dataset_indix
        )
        ...
        singular_value_list = (
            np.concatenate([singular_value_list, singular_value])
            if singular_value_list is not None
            else singular_value
        )
